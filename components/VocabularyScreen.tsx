...
const VocabularyScreen: React.FC<VocabularyScreenProps> = ({ unitNumber, vocabulary, onBack, classroomId, grade }) => {
    ...
    useEffect(() => {
        const preFetchAudio = async () => {
            ...
            for (const item of localVocabulary) {
                ...
                if (item.audio) continue; // Đã có âm thanh, bỏ qua
                ...
                try {
                    ...
                    if (error?.error?.code === 429) {
                        ...
                        break; // Dừng hàng đợi nếu bị giới hạn
                    }
                } finally {
                    if (isComponentMounted.current) {
                ...
                // Nghỉ ngắn giữa các lần gọi để tránh spam API
    ...
    const handlePlaySound = useCallback(async (wordItem: VocabularyWord, e: React.MouseEvent) => {
        ...
        try {
            ...
            // Nếu click mà vẫn chưa có audio (đang fetch hoặc fetch lỗi), thử fetch lại 1 lần duy nhất
            if (!base64Audio) {
            ...
            setTimeout(() => setErrorWord(null), 3000);
        }
