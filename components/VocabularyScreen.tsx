    ...
    return (
        <div className="w-40 h-40 bg-white rounded-2xl shadow-sm p-3 mb-4 flex items-center justify-center relative overflow-hidden">
            ...
            <img 
                src={src} 
                alt={alt} 
                className={`max-h-full max-w-full object-contain rounded-xl relative z-10 transition-opacity duration-500 ${loaded && !isPreloading ? 'opacity-100' : 'opacity-0'}`}
...
const VocabularyScreen: React.FC<VocabularyScreenProps> = ({ unitNumber, vocabulary, onBack, classroomId, grade }) => {
    ...
    // --- CƠ CHẾ SIÊU PRE-FETCHING ---
    // Tự động xếp hàng tải cả Ảnh (AI mô tả) và Âm thanh cho toàn bộ danh sách
    useEffect(() => {
        const preFetchAssets = async () => {
            ...
            for (const item of localVocabulary) {
                ...
                // 1. XỬ LÝ HÌNH ẢNH (Tải trước link AI xịn)
                // Nếu ảnh chưa có hoặc là link cơ bản, dùng AI tạo mô tả chuyên sâu
                const hasQualityImage = item.image && item.image.includes('illustrating');
                if (!hasQualityImage && !fetchingImages.has(item.word)) {
                    try {
                        ...
                        if (isComponentMounted.current) {
                            // Lưu vào database để người sau mở ra thấy ngay
                            ...
                            // Kích hoạt browser tải ảnh ngầm
                            const img = new Image();
                            img.src = betterImageUrl;
                        }
                    } catch (e) {
                        console.warn("Image pre-fetch failed for", item.word);
                    } finally {
                        if (isComponentMounted.current) {
                ...
                // 2. XỬ LÝ ÂM THANH (Tiếp tục hàng đợi tải âm thanh)
                ...
                // Nghỉ 1.5 giây giữa mỗi từ để ổn định API (Free tier)
        ...
        <div className="flex flex-col p-4 sm:p-6 bg-[#FFF8F0] min-h-[600px]">
            ...
            <div className="mb-4 flex items-center justify-center gap-4">
                {(fetchingImages.size > 0 || fetchingWords.size > 0) && (
                    <div className="bg-blue-50 px-4 py-1 rounded-full border border-blue-200 flex items-center gap-2 animate-pulse">
                        <div className="w-2 h-2 bg-blue-500 rounded-full animate-bounce"></div>
                        <span className="text-xs font-bold text-blue-700 uppercase">Đang tải trước học liệu AI...</span>
                    </div>
                )}
                {isRateLimited && (
                    <div className="bg-orange-50 px-4 py-1 rounded-full border border-orange-200 flex items-center gap-2">
                        <span className="text-xs font-bold text-orange-700">API Đang Bận - Chờ Giây Lát</span>
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 overflow-y-auto flex-grow px-2 pb-8 max-w-7xl mx-auto w-full">
                {localVocabulary.map((item, index) => {
                    ...
                    const imageUrl = item.image || `https://image.pollinations.ai/prompt/${item.word.replace(/\s+/g, '_')}_isolated_on_white_background?width=800&height=600&nologo=true`;
                    ...
                    return (
                        <div key={index} className={`${bgColor} rounded-[2rem] p-6 flex flex-col items-center shadow-sm hover:shadow-md transition-shadow duration-300`}>
                            <ImageWithLoader src={imageUrl} alt={item.word} isPreloading={isFetchingImage} />
