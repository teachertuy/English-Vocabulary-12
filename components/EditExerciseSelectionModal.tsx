...
const EditExerciseSelectionModal: React.FC<EditExerciseSelectionModalProps> = ({ show, onClose, onSave, currentConfig }) => {
    ...
    const handleUnitColorChange = (index: number, color: string) => {
        const newColors = [...(config.unitCardColors || DEFAULT_UNIT_COLORS)];
        newColors[index] = color;
        handleChange('unitCardColors', newColors);
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b flex justify-between items-center bg-gray-50 rounded-t-2xl">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 italic mt-1">Tùy chỉnh tiêu đề, nhãn nút và nội dung hoạt động</p>
                    </div>
                ...
                <div className="flex border-b bg-gray-50">
                    {[
                        ...
                        <button
                            ...
                            className={`flex-1 py-3 font-bold text-sm flex items-center justify-center gap-2 border-b-4 transition-all ${activeTab === tab.id ? 'border-blue-600 text-blue-600 bg-white' : 'border-transparent text-gray-500 hover:bg-gray-100'}`}
                        >
                ...
                <div className="flex-grow overflow-y-auto p-8 space-y-8 bg-white">
                    ...
                    {activeTab === 'units' && (
                        <div className="space-y-6 tab-content-enter">
                            <div className="p-6 border rounded-2xl bg-teal-50 border-teal-100">
                                <h3 className="font-bold text-teal-800 mb-6 flex items-center gap-2"><span>🔲</span> Kiểu dáng & Nhãn các ô UNIT / TOPIC</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Văn bản cho "UNIT"</label>
                                        <input type="text" value={config.unitLabelText} onChange={e => handleChange('unitLabelText', e.target.value)} className="w-full p-2 border rounded font-black text-teal-700" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Văn bản cho "TOPIC"</label>
                                        <input type="text" value={config.topicLabelText} onChange={e => handleChange('topicLabelText', e.target.value)} className="w-full p-2 border rounded font-black text-blue-700" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Văn bản nút "Thoát"</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Màu chữ số</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Màu nhãn loại ô (UNIT/TOPIC)</label>
                                    ...
                                    <div className="col-span-1 md:col-span-2 lg:col-span-3 grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-semibold text-gray-600 mb-1">Màu dòng kẻ 1 (Trắng)</label>
                                        ...
                                        <div>
                                            <label className="block text-sm font-semibold text-gray-600 mb-1">Màu dòng kẻ 2 (Vàng)</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Cỡ chữ số: <span className="text-teal-600 font-bold">{config.unitCardFontSize}rem</span></label>
                                        <input type="range" min="1" max="6" step="0.25" value={config.unitCardFontSize} onChange={e => handleChange('unitCardFontSize', parseFloat(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-teal-600" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Chiều cao ô: <span className="text-teal-600 font-bold">{config.unitCardHeight}rem</span></label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Độ bo góc: <span className="text-teal-600 font-bold">{config.unitCardBorderRadius}px</span></label>
                                ...
                                <div className="mt-8 border-t pt-6">
                                    <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2">🎨 Tùy chỉnh màu từng ô Unit (1-10)</h4>
                                    <div className="grid grid-cols-5 gap-4">
                                        {(config.unitCardColors || DEFAULT_UNIT_COLORS).map((color, idx) => (
                                            <div key={idx} className="flex flex-col items-center">
                                                <span className="text-[10px] font-bold text-gray-500 mb-1">Unit {idx + 1}</span>
                                                <input 
                                                    type="color" 
                                                    value={color} 
                                                    onChange={e => handleUnitColorChange(idx, e.target.value)} 
                                                    className="w-full h-10 p-0 border-0 cursor-pointer rounded overflow-hidden" 
                                                />
                                            </div>
                                        ))}
                                    </div>
                                    <p className="text-xs text-gray-400 mt-2 italic">* Các Topic (60 ô) sẽ lặp lại chu kỳ 10 màu này để nhìn khoa học hơn.</p>
                                </div>
                            </div>
                        </div>
                    )}
