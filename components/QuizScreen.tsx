...
const QuizScreen: React.FC<QuizScreenProps> = ({ playerData, questions, unitNumber, grade, onFinish, onForceExit, classroomId, activityId, onBack }) => {
    ...
    // Track student presence when component mounts
    useEffect(() => {
    ...
    // Update real-time progress whenever an answer is submitted
    useEffect(() => {
    ...
    const finishQuiz = useCallback(async (forceExit = false) => {
        ...
        const allDetails = questions.map((q, index) => {
        ...
        if (classroomId && activityId) {
            try {
                ...
                console.error("Failed to submit result and update presence in Firebase:", error);
            }
        }
    ...
    // Listen for teacher kicking the player
    useEffect(() => {
        if (!classroomId) return;
        const unsubscribe = listenForKickedStatus(classroomId, playerData.name, playerData.class, () => {
             finishQuiz(true);
        });
        return () => unsubscribe();
    }, [classroomId, playerData.name, playerData.class, finishQuiz]);

    // Handle visibility change for cheat detection
    useEffect(() => {
        const handleVisibilityChange = () => {
            if (document.hidden && classroomId) {
                incrementCheatCount(classroomId, playerData.name, playerData.class);
            }
        };
    ...
    if (!currentQuestion || !currentQuestion.options || !Array.isArray(currentQuestion.options) || !currentQuestion.sentence) {
        const handleExit = () => {
             const emptyResult: GameResult = {
                playerName: playerData.name,
                playerClass: playerData.class,
                score: '0',
                correct: 0,
        ...
        return (
            <div className="flex flex-col items-center justify-center p-8 text-center min-h-[600px] bg-orange-50">
                <h1 className="text-2xl font-bold text-red-600 mb-4">Lỗi Tải Câu Hỏi</h1>
                <p className="text-gray-700 mb-6">Không thể bắt đầu bài kiểm tra do có lỗi với dữ liệu câu hỏi. Vui lòng thử lại sau.</p>
                <button 
                    onClick={handleExit} 
                    className="bg-blue-600 text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-blue-700 transition shadow-lg"
                >
                    Quay lại
                </button>
            </div>
    ...
    const handleCheckAnswer = () => {
        if (!selectedAnswer) {
            setFeedback({ message: "Vui lòng chọn một đáp án!", isCorrect: false });
    ...
    return (
        <div className="flex flex-col items-center px-4 py-4 relative min-h-[600px] bg-orange-50">
             {feedback && (
                <div className={`fixed top-5 right-5 shadow-lg rounded-lg p-4 text-center z-50 transition-transform transform ${feedback.isCorrect ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            ...
            <div className="flex flex-col items-center w-full flex-grow">
                <div className="w-full max-w-3xl bg-white rounded-2xl border-4 border-green-500 px-2 py-4 shadow-lg flex flex-col flex-grow">
                    ...
                    <div className="w-full border-t-4 border-red-500 mb-4"></div>
                        ...
                        {currentQuestion.options.map((option, i) => {
                            ...
                            if (isAnswered) {
                                ...
                                if (option === currentQuestion.answer) {
                                     btnClass = 'p-3 rounded-lg text-left font-semibold text-base border-2 focus:outline-none transition bg-green-500 text-white border-green-700 cursor-not-allowed';
                                } else if (option === selectedAnswer) {
                                     btnClass = 'p-3 rounded-lg text-left font-semibold text-base border-2 focus:outline-none transition bg-red-500 text-white border-red-700 cursor-not-allowed';
                                }
                            } else if (option === selectedAnswer) {
                                btnClass = 'p-3 rounded-lg text-left font-semibold text-base border-2 focus:outline-none transition bg-orange-500 text-white border-orange-600';
                            }
                    ...
                    <div className="mt-4 flex justify-center space-x-4">
                         {!isAnswered ? (
                             <button onClick={handleCheckAnswer} className="bg-blue-600 text-white font-bold py-2 px-6 rounded-full text-lg hover:bg-blue-700 transition" disabled={!selectedAnswer}>Check Answer</button>
                         ) : (
                             <button onClick={handleNextQuestion} className="bg-amber-500 text-white font-bold py-2 px-6 rounded-full text-lg hover:bg-amber-600 transition">
